...
import views.editablecontroller.EditableIndex;
 ...
 * @param <T> 
 */
public class CommonPart<T> extends AvailableActivity {
     ...
     * CommonPart.vm.fxmlのChildPaneタグに、<!--% #render("NewComputer") -->
     * を記述している。
     */
    @Override
    ...
    @FXML private void handlePrevAction(ActionEvent event) {
        // 次に遷移するパス
        String path = "/ChildWindowController/newComputer";
    ...
    @FXML private void handleNextAction(ActionEvent event) {
        ...
        // 次に遷移するパス
        String path = "/ChildWindowController/selectCompany";
    ...
    @FXML private void handleCreateAction(ActionEvent event) {
        ...
        requestData("/ChildWindowController/createComputer")
                .params(params)
                .onSuccess(new CallBack<Object>() {
                    ...
                    public void call(Object data, Status status) {
                        ...
                        // 検索実行
                        ((EditableIndex)ownerActivity).searchAction(params);
                    }      
                })
                .execute();
